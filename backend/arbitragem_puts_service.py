...
class ArbitragemPutsService:
    ...
    def _criar_sessao_http(self) -> requests.Session:
        ...
        session.headers.update({
            'User-Agent': 'ArbitragemPutsService/1.0',
    ...
    def executar_screening(self, 
        ...
        try:
            ...
            logger.info(f"🔍 Iniciando screening arbitragem puts de {len(lista_ativos)} ativos...")
            ...
            logger.info(f"\n📊 Resumo do Screening Arbitragem Puts:")
            ...
            logger.error(f"❌ Erro geral no screening arbitragem puts: {e}")
            return {
                'success': False,
                'error': str(e),
                'timestamp': datetime.now().isoformat()
    ...
    def _analisar_puts(self, 
        ...
        """
        Análise de puts com lógica otimizada e validações robustas
        """
        try:
