...
class ExpirationManager:
    ...
    def test_data_availability(self, symbol, expiration_code):
        try:
            ...
            query = text("""
                SELECT COUNT(*) as total FROM opcoes_b3
...
class DataProvider:
    def __init__(self):
        ...
        logging.info("Conectando ao banco PostgreSQL (TEX)...")
        try:
    ...
    def get_floqui_oi_breakdown(self, symbol, expiration_code=None):
        try:
            if expiration_code:
                exp_meta = self.expiration_manager.available_expirations.get(expiration_code, {})
                days     = max(1, (exp_meta.get("date", datetime.now()) - datetime.now()).days) if exp_meta else 30
                expiration = {
                    "code":   expiration_code,
                    "desc":   exp_meta.get("desc", expiration_code),
                    "window": "SEMANAL" if "- W" in exp_meta.get("desc", "") else "MENSAL",
                    "days":   days
                }
            else:
            ...
            spot_price  = self.get_spot_price(symbol) or 100

            logging.info(f"TEX: spot={spot_price:.2f}, vencimento={expiration['desc']}")
...
class TEXCalculator:
    ...
    def calculate_tex(self, oplab_df, oi_breakdown, spot_price):
        ...
        for strike in valid_options['strike'].unique():
            ...
            call_tex = call_tex_desc = call_days = 0.0
            if call_data and len(calls) > 0:
                ...
                call_tex_desc = avg_theta * call_data['descoberto'] * 100
                call_days     = avg_days

            put_tex = put_tex_desc = put_days = 0.0
            if put_data and len(puts) > 0:
                ...
                put_tex_desc = avg_theta * put_data['descoberto'] * 100
                put_days     = avg_days

            # Dias ponderados por OI
            if call_data and put_data:
                total_oi    = call_data['total'] + put_data['total']
                weighted_days = (call_days * call_data['total'] + put_days * put_data['total']) / total_oi if total_oi > 0 else 0
            elif call_data:
            ...
            tex_data.append({
                ...
                'call_tex_descoberto':     float(call_tex_desc),
                'put_tex_descoberto':      float(put_tex_desc),
                'total_tex_descoberto':    float(call_tex_desc + put_tex_desc),
...
class TimeDecayRegimeDetector:
    ...
    def analyze_time_decay_regime(self, tex_df, spot_price):
        ...
        weighted_days = float(
            (tex_df['days_to_expiration'] * (tex_df['call_oi_total'] + tex_df['put_oi_total'])).sum() / total_oi
        ) if total_oi > 0 else float(tex_df['days_to_expiration'].mean())

        max_bleed_strike = float(tex_df.loc[tex_df['total_tex'].idxmin(), 'strike'])
...
class TEXAnalyzer:
    ...
    def create_6_charts(self, tex_df, spot_price, symbol, expiration_info=None):
        ...
        total_tex   = [float(x) for x in tex_df['total_tex'].tolist()]
        descoberto  = [float(x) for x in tex_df['total_tex_descoberto'].tolist()]
        call_tex    = [float(x) for x in tex_df['call_tex'].tolist()]
        put_tex     = [float(x) for x in tex_df['put_tex'].tolist()]

        colors1 = ['#7c2d12' if x < 0 else '#22c55e' for x in total_tex]
        fig.add_trace(go.Bar(x=strikes, y=total_tex, marker_color=colors1, showlegend=False), row=1, col=1)

        colors2 = ['#dc2626' if x < 0 else '#22c55e' for x in descoberto]
        fig.add_trace(go.Bar(x=strikes, y=descoberto, marker_color=colors2, showlegend=False), row=1, col=2)

        total_abs   = sum(abs(x) for x in total_tex)
        conc_pct    = [(abs(x)/total_abs*100) if total_abs > 0 else 0 for x in total_tex]
        risk_colors = ['#ef4444' if x > 20 else '#f97316' if x > 10 else '#22c55e' for x in conc_pct]
        fig.add_trace(go.Bar(x=strikes, y=conc_pct, marker_color=risk_colors, showlegend=False), row=2, col=1)

        fig.add_trace(go.Bar(x=strikes, y=call_tex, marker_color='#22c55e', showlegend=False), row=2, col=2)
        fig.add_trace(go.Bar(x=strikes, y=put_tex,  marker_color='#ef4444', showlegend=False), row=2, col=2)

        cumulative = list(np.cumsum(total_tex))
        fig.add_trace(go.Scatter(
            x=strikes, y=cumulative, mode='lines',
            line=dict(color='#06b6d4', width=4),
            fill='tozeroy', fillcolor='rgba(6,182,212,0.3)',
            showlegend=False
        ), row=3, col=1)

        days_remaining = expiration_info.get('days', 30) if expiration_info else 30
        velocity = [abs(d)/days_remaining if days_remaining > 0 else 0 for d in descoberto]
        vel_colors = ['#ef4444' if x > 1e7 else '#f97316' if x > 5e6 else '#eab308' for x in velocity]
        fig.add_trace(go.Bar(x=strikes, y=velocity, marker_color=vel_colors, showlegend=False), row=3, col=2)
    ...
    def create_single_chart(self, tex_df, spot_price, symbol,
                            expiration_info=None, mode='net'):    
        if tex_df.empty:
            return None

        tex_df  = tex_df.sort_values('strike').reset_index(drop=True)
        strikes = [float(x) for x in tex_df['strike'].tolist()]
        total_tex   = [float(x) for x in tex_df['total_tex'].tolist()]
        descoberto  = [float(x) for x in tex_df['total_tex_descoberto'].tolist()]
        call_tex    = [float(x) for x in tex_df['call_tex'].tolist()]
        put_tex     = [float(x) for x in tex_df['put_tex'].tolist()]
        call_oi     = [int(x)   for x in tex_df['call_oi_total'].tolist()]
        put_oi      = [int(x)   for x in tex_df['put_oi_total'].tolist()]

        mode_labels = {
            'net':        'Net Exposure',
            'descoberto': 'TEX Descoberto',
        ...
        if mode == 'net':
            colors = ['#7c2d12' if v < 0 else '#22c55e' for v in total_tex]
            fig.add_trace(go.Bar(
                x=strikes, y=total_tex,
                marker_color=colors, marker_line_width=0,
                showlegend=False,
                hovertemplate='Strike: R$ %{x:.2f}<br>TEX: %{y:,.0f}<extra></extra>'
            ))

        elif mode == 'descoberto':
            colors = ['#dc2626' if v < 0 else '#22c55e' for v in descoberto]
            fig.add_trace(go.Bar(
                x=strikes, y=descoberto,
                marker_color=colors, marker_line_width=0,
                showlegend=False,
                hovertemplate='Strike: R$ %{x:.2f}<br>TEX Desc: %{y:,.0f}<extra></extra>'
            ))

        elif mode == 'call_put':
            fig.add_trace(go.Bar(
                x=strikes, y=call_tex, name='Calls',
                marker_color='#22c55e', marker_line_width=0,
                hovertemplate='Strike: R$ %{x:.2f}<br>Call TEX: %{y:,.0f}<extra></extra>'
            ))
            fig.add_trace(go.Bar(
                x=strikes, y=put_tex, name='Puts',
                marker_color='#ef4444', marker_line_width=0,
                hovertemplate='Strike: R$ %{x:.2f}<br>Put TEX: %{y:,.0f}<extra></extra>'
            ))
    ...
    def create_cumulative_chart(self, tex_df, spot_price, symbol, expiration_info=None):
        """TEX Cumulativo — sangramento acumulado ao longo dos strikes."""
        if tex_df.empty:
            return None
        ...
        fig.add_trace(go.Scatter(
            ...
            showlegend=False,
            hovertemplate='Strike: R$ %{x:.2f}<br>TEX Cum: %{y:,.0f}<extra></extra>'
        ))
    ...
    def analyze(self, symbol, expiration_code=None):
        ...
        # Corrige dias com valor real do vencimento
        ...
        single_charts = {
            'net':        self.create_single_chart(tex_df, spot_price, symbol, expiration_info, mode='net'),
            'descoberto': self.create_single_chart(tex_df, spot_price, symbol, expiration_info, mode='descoberto'),
            'call_put':   self.create_single_chart(tex_df, spot_price, symbol, expiration_info, mode='call_put'),
            'oi':         self.create_single_chart(tex_df, spot_price, symbol, expiration_info, mode='oi'),
            'cumulativo': self.create_cumulative_chart(tex_df, spot_price, symbol, expiration_info),
        }
        ...
        logging.info(f"TEX Total:  {decay_regime.get('total_tex', 0):,.0f}")
        logging.info(f"Strikes:    {len(tex_df)}")
        ...
        return {
            ...
            'chart_data':       tex_df.to_dict(orient='list'),  # NOVO
...
class ThetaService:
    ...
    def analyze_theta_complete(self, ticker, expiration_code=None, days_back=60):
        try:
            ...
            # Recalcula pressão com base nos dias reais
