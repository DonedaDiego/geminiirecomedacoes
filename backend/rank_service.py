...
class RankingService:
    """
    Serviço limpo para ranking de volatilidade implícita
    """
    ...
    def _get_token(self):
        """Busca token de múltiplas fontes"""
    ...
    def fetch_data(self, rank_by="iv_current", sort="desc", limit=100):
        ...
        try:
            ...
            print(f"🔍 Buscando dados: {rank_by}")
    ...
    def get_percentil_data(self, df, top_n=20):
        """Prepara dados para ranking por percentil"""
        if df is None or df.empty:
            return [], {}
        
        # Ordenar por percentil se disponível
        if 'iv_6m_percentile' in df.columns:
            df = df.sort_values('iv_6m_percentile', ascending=False)
        
        ranking = []
        categorias = {'criticos': 0, 'altos': 0, 'medios': 0, 'baixos': 0}
        
        for idx, (_, row) in enumerate(df.head(top_n).iterrows(), 1):
            percentil = float(row.get('iv_6m_percentile', 0))
